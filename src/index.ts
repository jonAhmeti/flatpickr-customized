...
function FlatpickrInstance(
  ...
  function build() {
    ...
    if (self.config.customButtons && self.config.customButtons.length > 0) {
      self.customButtonsContainer = createElement<HTMLDivElement>(
        "div",
        "flatpickr-custom-buttons-container"
      );
      fragment.appendChild(self.customButtonsContainer);

      self.config.customButtons.forEach((customButton) =>
        buildCustomButton(customButton)
      );
    }
  ...
  function buildCustomButton(customButton: CustomButton) {
    const btn = self._createElement<HTMLButtonElement>(
      "button",
      "", // The CSS class name
      customButton.text
    );

    if (customButton.classes) {
      customButton.classes.forEach((c) => btn.classList.add(c));
    }

    btn.type = "button";

    btn.addEventListener("click", () => customButton.callback(self));
    self.customButtonsContainer.appendChild(btn);
  }
  ...
  function createDay(
    ...
    dayElement.setAttribute(
        "data-date",
        self.formatDate(date, "Y-m-d")
    );
    if (self.config.minDate && compareDates(date, self.config.minDate, true) < 0)
      dayElement.classList.add("flatpickr-below-minDate");
